...
def plot_numeric_feature_histogram(X: pd.DataFrame, num_features: List[str], task_type: str):
    ...
    top_num_features = num_features[:6]
    n_cols = 2
...
def main(csv_path: str, label_col: str):
    ...
    try:
       ...
       #     continue  # 跳过当前模型，继续处理下一个 
        ...
        # 保存为CSV文件
        performance_df.to_csv(f'CHDDG34_model_performance_summary_{task_type}.csv', index=False, encoding='utf-8-sig')
        print(f"\nPerformance summary saved to: CHDDG34_model_performance_summary_{task_type}.csv")
        ...
        print(f"Test set ID + predictions saved to: CHDDG34_test_set_id_predictions_{task_type}.csv")

        print("="*50)
        print("All processes completed successfully!")
        print(f"\nKey output files:")
        print(f"  1. Performance summary: CHDDG34_model_performance_summary_{task_type}.csv")
        print(f"  2. Test set ID + predictions: CHDDG34_test_set_id_predictions_{task_type}.csv")
...
if __name__ == "__main__":
    # 替换为你的数据集路径和标签列
    CSV_FILE_PATH = "/data/ruth/rrl2/dataset/FREET_E_PT.csv"
    LABEL_COLUMN = "CLASS"
